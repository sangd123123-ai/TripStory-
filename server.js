// server.js ??ë©”ì¸ ?°ì„  êµ¬ì¡° + subserver ê¸°ëŠ¥ ?©ë¥˜
...
// ====== ?¤í‚¤ë§?? ë“±ë¡?(?ˆìœ¼ë©?ë¡œë“œë§?/ ?†ì–´???¤ë¥˜ ?†ì´ ?¤í‚µ) ======
[
  './models/tripSchema',
  './models/mytripSchema',
  './models/imageSchema',
  ...
  './models/reviewSchema',
  './models/TravelReview',
  './models/Comment', // subserver?ì„œ ?¬ìš©
  './models/tripSchema',
...
// ====== ?ˆì „ ë§ˆìš´??? í‹¸: ?¨ìˆ˜??app) & Router ????ì§€??======
function isRouterLike(mod) {
  // express.Router()??function?´ë©´??use/handle/stack ?ì„±??ê°€ì§?
  return (
    mod &&
...
function mountAuto(basePathOrApp, modPath) {
  try {
    ...
    console.warn(`? ï¸ mount skipped (unknown export): ${modPath}`);
  } catch (e) {
    console.warn(`? ï¸ mount skipped ${modPath}: ${e.message}`);
  }
}

// ===== (A) ë©”ì¸ ?¼ìš°??(?°ì„ )
mountAuto('/auth', './routers/auth');                 // ???¬ê¸°???ë™?ë³„
...
try {
  ...
  console.warn('? ï¸ /notices skipped:', e.message);
}

// ===== (B) subserver ê¸°ëŠ¥ (?„ìˆœ??
...
app.get("/_envcheck", (req, res) => {
  ...
  res.json({
    KMA_KEY: process.env.KMA_SERVICE_KEY ? "??OK" : "???†ìŒ",
    KTO_KEY: process.env.KTO_SERVICE_KEY ? "??OK" : "???†ìŒ",
    NODE_ENV: process.env.NODE_ENV,
  });
});


// ====== React ?•ì  ë¹Œë“œ ?œë¹™ (?ˆì„ ?Œë§Œ) ======
try {
  ...
  console.warn('? ï¸ build serve skipped:', e.message);
}
